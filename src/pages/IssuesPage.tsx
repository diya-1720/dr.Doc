    ...
    <div className="min-h-screen bg-[#F3E4C8] flex flex-col md:flex-row">
      ...
      <main className="flex-1 p-6 md:p-8 max-w-6xl">
        ...
        {filteredIssues.length === 0 ? (
          <div className="bg-[#FFF8EA] p-8 border-2 border-[#3F2928] text-center font-mono text-xs">
          ...
          <div className="space-y-6">
            {filteredIssues.map((issue) => {
              ...
              return (
                <div
                  key={issue.id}
                  className={`bg-[#FFF8EA] p-6 border-2 shadow-[4px_4px_0px_#3F2928] relative ${
                    issue.resolved
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-mono text-xs mb-6">
                    ...
                    <div className="p-4 bg-[#E8B9B8] border border-[#7A302F]">
                      <div className="font-bold text-[#7A302F] mb-1 uppercase flex items-center gap-1.5">
                  ...
                  {/* Action Buttons */}
                  {!issue.resolved && (
                    <div className="flex flex-wrap items-center gap-3 pt-3 border-t border-[#3F2928]/20 font-mono text-xs">
                      <button
                        onClick={handleFixClick}
                        className="bg-[#7A302F] hover:bg-[#5c2322] text-[#FFF8EA] px-5 py-2 border border-[#3F2928] shadow-[2px_2px_0px_#3F2928] font-heading text-base font-bold flex items-center gap-2 active:translate-x-[1px] active:translate-y-[1px] transition-all"
                      >
                      ...
                      <button
                        ...
                        className="bg-[#FFF8EA] hover:bg-[#E8B9B8] text-[#3F2928] px-4 py-2 border border-[#3F2928] shadow-[2px_2px_0px_#3F2928] font-mono text-xs font-bold flex items-center gap-2"
                      >
                        <MapPin className="w-4 h-4 text-[#7A302F]" />
                        FIND HELP NEARBY (OFFLINE SERVICE)
                      </button>

                      <button
                        onClick={() => resolveIssue(issue.id)}
                        className="ml-auto text-[#A58B7B] hover:text-[#7A302F] underline font-bold"
                      >
                        MARK AS RESOLVED
                      </button>
                    </div>
